...
export default function AnalysisDashboard() {
  ...
  const handleDownload = () => {
    ...
    const doc = new jsPDF();
    doc.text("CV Analysis Results", 10, 10);
    doc.text(`Overall Score: ${results?.overallScore}%`, 10, 20);

    const addSection = (title: string, section: AnalysisSection, yOffset: number) => {
      doc.text(title, 10, yOffset);
      const keyPoints = extractKeyPoints(section.summary);
      keyPoints.forEach((point, index) => {
        doc.text(`- ${point}`, 10, yOffset + 10 + index * 10);
      });
      const recommendations = extractRecommendations(section.recommendations);
      if (recommendations.length > 0) {
        doc.text("Recommendations:", 10, yOffset + 10 + keyPoints.length * 10);
        recommendations.forEach((rec, index) => {
          doc.text(`* ${rec}`, 10, yOffset + 20 + keyPoints.length * 10 + index * 10);
        });
      }
    };

    addSection("Skills", results!.skills, 30);
    addSection("Experience", results!.experience, 100);
    addSection("Education", results!.education, 170);
